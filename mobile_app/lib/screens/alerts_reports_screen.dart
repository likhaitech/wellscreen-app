                ...
                return ListView(
                  ...
                  children: [
                    ...
                    const AlertReportCard(
                      ...
                      subtitle:
                          'Live now, three mechanisms in order: captured '
                          'domains are first checked on-device '
                          '(SiteCategoryService) against the real cleaned '
                          'dataset (gambling, drugs, dangerous_material, '
                          'adult - 58,219 real domains, see '
                          'ml/site_category/README.md - drugs alone grew '
                          'to 25,948 real domains this round after adding '
                          'a second real source). If that finds nothing, '
                          'a keyword check (AdultKeywordDetector) flags '
                          'domains containing pornography-related terms '
                          '(e.g. "porn", "xxx", "sex", "tube"), the '
                          'technique described in our manuscript for that '
                          'category, guarded by a curated allowlist for '
                          'known-safe domains that would otherwise collide '
                          '(e.g. youtube.com) - entries it catches show '
                          '"(keyword match)" above. If that also finds '
                          'nothing, a real trained ML classifier '
                          '(category_classifier_weights.json, char n-gram '
                          'TF-IDF + logistic regression, trained on 78,177 '
                          'real domains including a real safe-domain class) '
                          'checks whether the domain LOOKS like a gambling '
                          'site even if it is not literally in the dataset - '
                          'entries it catches show "(AI-detected)" above. '
                          'That fallback is deliberately restricted to '
                          'gambling only, above a 90% confidence bar '
                          'measured against 200 held-out real safe domains '
                          '(0 false positives at that bar, 89.3% recall on '
                          'real held-out test domains - see '
                          'ml/site_category/output/'
                          'category_classifier_evaluation.txt). drugs was '
                          'also tested for this after its dataset grew, '
                          'but deliberately held back: a harder, targeted '
                          'stress test against real pharmacy/healthcare '
                          'domains found it false-positives on legitimate '
                          'companies (e.g. truepill.com at 95.1% '
                          'confidence, even after a follow-up data-cleaning '
                          'pass removed 7 verified mislabeled domains from '
                          'training) - a real, reproducible risk we '
                          'chose not to ship. dangerous_material stays '
                          'exact-match-only too, since there are too few '
                          'real training examples (35) for the model to '
                          'be reliable there, and no larger real source '
                          'was found after searching. A match from any of '
                          'the three shows a category tag in Recent '
                          'Browsing Activity above and sends the parent a '
                          'push alert. This is detection and after-the-fact '
                          'alerting, not blocking - the page still loads, '
                          'since real-time prevention would need this same '
                          'check running natively before the page renders, '
                          'which isn\'t built yet. No self-harm category '
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        ...
                        '${entry['detectionSource'] == 'ml' ? ' (AI-detected)' : entry['detectionSource'] == 'keyword' ? ' (keyword match)' : ''} · '
                        '${_formatTimestamp(entry['timestampMs'])}',
                        style: TextStyle(
                          color: entry['category'] != null
                              ? Colors.redAccent
