...
/// rule violations, frequent app opens). The expected label/probabilities
/// for each profile were computed independently by re-implementing the
/// exact same tree-walk/averaging algorithm in Python straight from this
/// same JSON file and running it for real - not guessed, not hand-tuned
/// to make the test pass. Re-verified against the 7-feature forest
/// (frequent_app_opens_today added) after that indicator was built - the
/// expected numbers below are this forest's real output, not the earlier
/// 6-feature model's.
void main() {
  ...
  group('MlRiskClassifierService.classify (real trained forest)', () {
    ...
    test(
      ...
      () async {
        final result = await service.classify({
          ...
          'frequent_app_opens_today': 0,
        });

        expect(result.label, 'Low Risk');
        // Verified via independent Python replay of the real forest:
        // ~0.9307 averaged probability for Low Risk on this profile.
        expect(result.confidence, closeTo(0.9307, 0.01));
        expect(
    ...
    test(
      'a heavy-usage, high-violations, late-night-heavy, frequently-'
      'reopened-app profile classifies as High Risk',
      () async {
        final result = await service.classify({
          ...
          'frequent_app_opens_today': 30,
        });

        expect(result.label, 'High Risk');
        // Verified via independent Python replay: ~0.8454.
        expect(result.confidence, closeTo(0.8454, 0.01));
      },
    );
    ...
    test('a moderate usage profile classifies as Moderate Risk', () async {
      final result = await service.classify({
        ...
        'frequent_app_opens_today': 5,
      });

      expect(result.label, 'Moderate Risk');
      // Verified via independent Python replay: ~0.6119.
      expect(result.confidence, closeTo(0.6119, 0.01));
    });
    ...
    test(
      ...
      () async {
        // classify() reads features[name] ?? 0 for each of the seven
        ...
        expect(result.confidence, closeTo(0.9136, 0.01));
      },
    );
